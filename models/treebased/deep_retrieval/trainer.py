...
def main(args):
    ...
    em_execution_interval = config.get("hyper_parameters.em_execution_interval", 2)
    pernal_factor = config.get("hyper_parameters.pernalize_factor", 1e-7)
    pernal_uploops = config.get("hyper_parameters.pernal_uploops", 3)
    poly_power = config.get("hyper_parameters.poly_power",3)
    # print("----em_execution_interval", em_execution_interval)
    ...
    if isinstance(pernal_factor, str):
        pernal_factor = eval(pernal_factor)
    ...
    for epoch_id in range(last_epoch_id + 1, epochs):
        ...
        if record_item_to_user_emb:
            ...
            if data_format == "amazon_book_behavior":
                # print("----item_to_user_seq_item:",item_to_user_seq_item) # {key:[array[...]], ...}
                for key in item_to_user_seq_item: 
                    ...
                    # print("=======user_seq========",user_seq) # [[],[]]
                    path,pro = dy_model.generate_candidate_path_for_item(user_seq, beam_size, is_static = False)   # epoch = 3 ; batchId=0;
                ...
                for key in item_to_user_emb:
                    ...
                    path,pro = dy_model.generate_candidate_path_for_item(user_emb,beam_size,is_static=True)   # epoch = 3 ; batchId=0;
            ...
            if pernalize_path_to_item_count:
                ...
                dy_model_class.graph_index.update_Jpath_of_item(item_to_path_map,item_to_path_score_map, pernal_uploops, pernal_factor, poly_power)
